    ...
    <div>
      <Suspense>
        <MenuButton v-if="user"
          ...
          <span class="ml-3 hidden text-sm font-medium lg:block"><span class="sr-only">Open user menu for </span>{{
              user.name
          }}</span>
          <ChevronDownIcon class="ml-1 hidden h-5 w-5 flex-shrink-0 lg:block" aria-hidden="true" />
        </MenuButton>
        ...
        <template #fallback>
          <MenuButton v-if="user"
            ...
            <div class="block bg-aronium-500 animate-pulse h-8 w-8 rounded-full" />
            <span class="ml-3 hidden text-sm font-medium lg:block"><span class="sr-only">Open user menu for
              </span>Loading...</span>
            <ChevronDownIcon class="ml-1 hidden h-5 w-5 flex-shrink-0 lg:block" aria-hidden="true" />
          </MenuButton>
        </template>
