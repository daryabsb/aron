        ...
        <Suspense>
          <RadioGroup v-model="selectedOrderItem">
            <div class="border-b border-aronium-500 divide-y divide-aronium-500 overflow-y-auto scrollbar">
              <RadioGroupOption v-for="item in useActiveOrder.items" :key="item" as="template"
                v-slot="{ checked, active }" :value="item">
                <div :class="[
                  checked
                  ...
                  <div class="w-40 text-aronium-white">
                    {{ store.useActiveOrder.itemDiscountPerItem(item) }}+{{ store.useActiveOrder.calculateItemDiscount(item) }}
                    <!-- Right buttons -->
