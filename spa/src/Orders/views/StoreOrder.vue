<template>
  ...
  <div class="flex h-screen">
    <!-- Narrow sidebar -->
    <div class="hidden w-24 overflow-y-auto bg-aronium-800 border-r border-aronium-500 md:block">
      <div class="flex w-full flex-col items-center py-6">
        <div class="flex flex-shrink-0 items-center">
          <img class="h-8 w-auto" src="https://tailwindui.com/img/logos/mark.svg?color=white" alt="Your Company" />
        </div>
        <div class="mt-6 w-full flex-1 space-y-1 px-2">
          <a v-for="item in sidebarNavigation" :key="item.name" @click="item.submit" :class="[
            item.current
      ...
      <Dialog as="div" class="relative z-20 md:hidden" @close="mobileMenuOpen = false">
        ...
        <div class="fixed inset-0 z-40 flex">
          <TransitionChild as="template" enter="transition ease-in-out duration-300 transform"
            ...
            <DialogPanel class="relative flex w-full max-w-xs flex-1 flex-col bg-aronium-800 pt-5 pb-4">
              ...
              <div class="mt-5 h-0 flex-1 overflow-y-auto px-2">
                <nav class="flex h-full flex-col">
                  <div class="space-y-1">
                    <span v-for="item in sidebarNavigation" :key="item.name" @click="item.submit" :class="[
                      item.current
                        ? 'bg-aronium-900 text-white'
    ...
    <div class="flex flex-1 flex-col overflow-hidden">
      <header class="w-full">
        <div class="relative z-10 flex h-16 flex-shrink-0 border-b border-aronium-500 bg-aronium-300 shadow-sm">
          <button type="button"
          ...
          <div class="flex flex-1 justify-between px-4 sm:px-6">
            ...
            <div class="ml-2 py-2 flex items-center space-x-4 sm:ml-6 sm:space-x-6">
              <button type="button"
...
const sidebarNavigation = [
  { name: "Home", submit: () => router.push("/"), icon: HomeIcon, current: false },
  {
    name: "Search",
