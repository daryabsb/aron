...
const getters = {
  ...
  [GET_ACTIVE_ORDER](state) {
    return state.cart.find((order) => order.number === state.activeOrderNumber);
  },
  [GET_CART_INDEX]: (state) => (product) => {
    const activeOrder = state.cart.find(
      (item) => item.id === activeOrderNumber.value
    );
    if (!activeOrder) return -1;
    return state.cart[activeOrder].items.findIndex((p) => p.id === product.id);
  },
  [GET_KEYWORD](state) {
